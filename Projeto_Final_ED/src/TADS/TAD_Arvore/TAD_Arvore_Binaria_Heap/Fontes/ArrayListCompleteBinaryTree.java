...
import TADS.TAD_Arvore.TAD_Arvore_Binaria_Heap.Interfaces.CompleteBinaryTree;
...
public class ArrayListCompleteBinaryTree<TYPE> implements CompleteBinaryTree<TYPE> {
	...
	public int depth(ArrayListCompleteBinaryTree<TYPE> tree, Position<TYPE> position) {
		
		if(isRoot(position)) {return 0;} //Verifica se posição informada é a raiz da arvore, caso seja retorna 0.
		else {return 1 + depth(tree, parent(position));} //Caso contrario, chama recursivamente até encontrar a raiz, sempre somando +1 e retorna essa soma.		
	}
